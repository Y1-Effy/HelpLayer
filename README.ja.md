...
`engines` は **Node.js ≥ 18** を要求しますが、**これはツール向けの要件だけ — CLI（`help-layer check` /
`scaffold`）・ビルド・npm パッケージのインストール用**です。ライブラリ本体はブラウザで動作し、実行時に
Node.js は不要です。ページで ESM／IIFE ビルドを読み込むのに Node 依存はありません。

## 既知の制約

- closed な Shadow DOM は JS から到達できないため非対応（open のみ貫通）。
- ホスト接続**後**に `attachShadow()` された open shadow root は自動検出されません。`MutationObserver` が見るのは
  ホストの `childList` であって、後からの shadow attach ではないためです。Web コンポーネントが非同期に
  アップグレード／初期化して shadow root を張る場合、再走査するまでその中の対象にはマーカーが付きません —
  コンポーネント初期化後に `update(config)`（teardown→再構築し、現在の open shadow root を貫通し直す）を呼ぶか、
  モードを OFF→ON してください。
- **1 つの document につき HelpLayer インスタンスは 1 つ**のみ対応します。2 つ同時に有効化すると document
  単位のリソース（`inert` による a11y 隔離、注入する `<style>`、`window.helpLayerDiagnose` グローバル）を
  奪い合うため、別のインスタンスが生存中に 2 つ目を初期化すると警告を出します。再初期化の前に前のインスタンスで
  `destroy()` を呼んでください。（別 document／iframe は互いに独立です。）
- `--help-layer-marker-size` のカスタム値は**反映されます** — マーカーは実行時に実際のレンダリングサイズを
  読み取るため、サイズを変えても対象に正しく吸着します。唯一の注意点：サイズは一度だけ計測してキャッシュするため、
  マーカー配置**後**に変数を変えても remount（OFF→ON か `update(config)`）まで再計測されません。
...
- **「ホストのイベントに触れない」の意味。** HelpLayer はホストアプリ自身のイベントリスナーを削除・置換・
  ラップしません。ON 中は透明な遮断レイヤーがポインタ入力やキー操作を吸収し、通常操作を防ぎます。これは
  ホストが**一切のイベントを観測しない**保証ではありません — HelpLayer 開始前に `window`／`document` の
  **キャプチャフェーズ**へ登録されたグローバルリスナーは、レイヤーが抑止する前に動作し得ます。「既存ハンドラは
  そのまま残し通常操作を遮断する」ものであり、「いかなるイベントも観測され得ない」ものではないと捉えてください。
